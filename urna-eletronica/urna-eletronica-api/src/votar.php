...
<?php
	...
	if(!isset($_SESSION['usuario']) && !isset($_SESSION['titulo']) && !isset($_SESSION['nome'])){
		header("Location: ./?erro=1");
	}

	$nTitEleitor = $_SESSION['titulo'];
	$nomeEleitor = $_SESSION['nome'];
	...
	if(isset($_GET['nTitEleitor']) && isset($_GET['nNum'])){
		...
		echo "<a href='./votar.php'>Votar novamente<a/>";
	}
	else{
		echo "<a href='sair.php'>Sair<a/></br>";
	}
...
<html lang="pt-BR">
	...
	<body>
		...
		<form method="get" class="main flex">
			<?php
				echo "<div>Nº título <input readonly type='number' name='nTitEleitor' value='".$nTitEleitor."'/></div></br>";
				echo "<div>Nome <input disabled type='text' name='nomeEleitor' value='".$nomeEleitor."'/></div>";
			?>
					...
					<?php
						if(isset($_GET['nTitEleitor']) && isset($_GET['nNum'])){
							...
							<div class="flex foto-nome">
								<img src="imagens/candidato.jpg" width="150px"/>
								<div id="nome"></div>
							</div>
							</div>
